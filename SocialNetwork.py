...
class SocialNetwork:
    ...
    def __new__(cls, name):
        # if we have already create a network, we can't create on more and we return the first network
    ...
    def sign_up(self, user_name, password):
        if user_name not in self.users:  # check that we don't have an user with the same name
            if 4 <= len(password) <= 8:  # check that the length of the password is ok
